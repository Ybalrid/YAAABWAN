...
//
// Ybalrid 02/02/2020
// Replaced TinyWireM with Wire so that it could be used on an Arduino Nano instead of a Trinket
//
//--------------------------------------------------------------------
//
...
#define Z_PROBE_SENSITIVITY 18 // Based on Repetier's config value
...
{
  Wire.beginTransmission(IIS2DH_ADDRESS);
  Wire.write(val);
...
void accelerometer_recv(uint8_t reg)
{
  ...
  Wire.requestFrom(IIS2DH_ADDRESS, 1); // request one 8 bit response
